...
export default {
  ...
  copyText: {
    color: ({ background }) => (chroma(background).luminance() >= 0.7 ? '#000' : '#fff')
  },
  colorName: {
    color: ({ background }) => (chroma(background).luminance() <= 0.08 ? '#fff' : '#000')
  },
  seeMore: {
    color: ({ background }) => (chroma(background).luminance() >= 0.7 ? 'rgba(0,0,0,0.6)' : '#fff'),
